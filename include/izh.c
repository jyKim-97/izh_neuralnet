...
// bidirectional network creation
void init_bi_ntk(int num_pre, int num_post, ntk_t *ntk)
{
    ...
    for (int n=0; n<num_pre; n++){
        ...
        ntk->strength[n] = (double*) malloc(_block_size * sizeof(double));
    }
}
...
void append_node(int node_id, int *n_edge, int **adj_list)
{
    ...
    if ((*n_edge > 0) && (*n_edge % _block_size == 0)){ // increase the size
        *adj_list = (int*) realloc(*adj_list, *n_edge + _block_size);
    }
...
void append_value(double val, int *n_edge, double **val_list)
{
    // adj_list: pointer of the n th adj list
    if ((*n_edge > 0) && ((*n_edge)%_block_size == 0)){ // increase the size
        *val_list = (double*) realloc(*val_list, *n_edge + _block_size);
    }
    (*val_list)[*n_edge] = val;
    (*n_edge)++;
}
