...
void run(double tmax, WRITER_VAR save_var_types){
    neuron_t cells;
    syn_t syns, bck_syns;
    init_simulation(&cells, &syns, &bck_syns);

    int max_step = tmax/_dt;

    writer_t fp_obj;
    init_writer(&fp_obj, env_tag, save_var_types);
    write_env(&fp_obj, cells.num_cells, bck_syns.num_pres, tmax, _dt, cells.types);
...
void init_simulation(neuron_t *cells, syn_t *syns, syn_t *bck_syns){
    network_info_t info = {0,};
    set_env(&info);
    init_network(&info, cells, syns, bck_syns);
    set_cell_params(cells);
    export_env(env_tag, &info, 1, "random_seed", (double) seed);
}
